...
def create_app(conexao=None, config=settings, notificador=None):
    """Application factory. `conexao` e `notificador` são injetáveis para teste."""
    config.validar()
    ...
    controllers = {
        "produto": ProdutoController(produtos),
        "usuario": UsuarioController(usuarios, config.SECRET_KEY, config.TOKEN_TTL_HORAS),
        "pedido": PedidoController(pedidos, notificador or NotificadorLog()),
        "relatorio": RelatorioController(relatorios),
        "health": HealthController(produtos, usuarios, pedidos, config.VERSAO, config.AMBIENTE),
    }
