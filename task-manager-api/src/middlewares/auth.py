...
def gerar_token(usuario, segredo, ttl_horas=8):
    payload = {'sub': usuario.id, 'role': usuario.role, 'exp': (datetime.now(timezone.utc) + timedelta(hours=ttl_horas)).timestamp()}
...
def requer_papel(papel):
    ...
    def decorator(fn):
        ...
        @wraps(fn)
        def wrapper(*args, **kwargs):
            if getattr(g, 'usuario', {}).get('role') != papel:
                raise Proibido('Permissão insuficiente')
            return fn(*args, **kwargs)
        return wrapper
    return decorator

def proteger_se_exigido(view, config):
    if not getattr(config, 'AUTH_REQUIRED', False):
        return view
    return requer_autenticacao(config.SECRET_KEY)(view)
