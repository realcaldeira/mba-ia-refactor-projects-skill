"""Casos de uso de task. Orquestra validação e model — sem ORM e sem regra de domínio."""
import logging
...
class TaskController:
    ...
    def criar(self):
        ...
        tarefa = Task(
            ...
            due_date=validators.converter_data(dados.get("due_date")),
            tags=validators.normalizar_tags(dados.get("tags")),
        )
    ...
    def atualizar(self, task_id):
        ...
        if "due_date" in dados:
            tarefa.due_date = validators.converter_data(
                dados["due_date"], mensagem="Formato de data inválido"
            )
        if "tags" in dados:
    ...
    def pesquisar(self):
        tarefas = Task.buscar_com_filtros(
