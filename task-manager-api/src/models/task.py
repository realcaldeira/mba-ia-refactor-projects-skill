...
class Task(db.Model):
    ...
    def to_dict(self, com_relacionamentos=False):
        dados = {'id': self.id, 'title': self.title, 'description': self.description, 'status': self.status, 'priority': self.priority, 'user_id': self.user_id, 'category_id': self.category_id, 'created_at': str(self.created_at), 'updated_at': str(self.updated_at), 'due_date': str(self.due_date) if self.due_date else None, 'tags': self.tags.split(',') if self.tags else []}
    ...
    def resumo(self):
        return {'id': self.id, 'title': self.title, 'description': self.description, 'status': self.status, 'priority': self.priority, 'created_at': str(self.created_at), 'due_date': str(self.due_date) if self.due_date else None, 'overdue': self.is_overdue()}
    ...
    def buscar_com_filtros(cls, termo=None, status=None, prioridade=None, user_id=None):
        ...
        if termo:
            consulta = consulta.filter(db.or_(cls.title.like(f'%{termo}%'), cls.description.like(f'%{termo}%')))
        if status:
    ...
    def contagem_por_status(cls):
        linhas = db.session.execute(db.select(cls.status, db.func.count(cls.id)).group_by(cls.status)).all()
        return {status: total for status, total in linhas}

    @classmethod
    def contagem_por_prioridade(cls):
        linhas = db.session.execute(db.select(cls.priority, db.func.count(cls.id)).group_by(cls.priority)).all()
        return {prioridade: total for prioridade, total in linhas}

    @classmethod
    def atrasadas(cls):
        candidatas = db.session.execute(db.select(cls).filter(cls.due_date.isnot(None), cls.status.notin_(STATUS_FINAIS))).scalars().all()
        return [t for t in candidatas if t.is_overdue()]

    @classmethod
    def criadas_desde(cls, momento):
        return db.session.execute(db.select(db.func.count(cls.id)).filter(cls.created_at >= momento)).scalar_one()

    @classmethod
    def concluidas_desde(cls, momento, status_concluida):
        return db.session.execute(db.select(db.func.count(cls.id)).filter(cls.status == status_concluida, cls.updated_at >= momento)).scalar_one()

    @classmethod
    def produtividade_por_usuario(cls, status_concluida):
        linhas = db.session.execute(db.select(cls.user_id, db.func.count(cls.id), db.func.sum(db.case((cls.status == status_concluida, 1), else_=0))).group_by(cls.user_id)).all()
