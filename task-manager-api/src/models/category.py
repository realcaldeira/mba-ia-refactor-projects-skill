...
class Category(db.Model):
    ...
    def to_dict(self, task_count=None):
        dados = {'id': self.id, 'name': self.name, 'description': self.description, 'color': self.color, 'created_at': str(self.created_at)}
    ...
    def contagem_de_tasks(cls):
        ...
        linhas = db.session.execute(db.select(Task.category_id, db.func.count(Task.id)).group_by(Task.category_id)).all()
